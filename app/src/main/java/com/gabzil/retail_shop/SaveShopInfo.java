 ...
 */
public class SaveShopInfo extends AsyncTask<Object, String, String>
{
    private Context mContext;
    ProgressDialog mProgress;
    private OnTaskCompleted mCallback;

    public SaveShopInfo(Context context, OnTaskCompleted listner)
    {
    ...
    {
        if (!isCancelled()){
            ShopDBEntities shop = (ShopDBEntities) params[0];
            return getServerInfo(shop);
        }
        else
            return null;
    }
    ...
    private String getServerInfo(ShopDBEntities shop)
    {
        Gson gson = new Gson();
        String ShopString = gson.toJson(shop);
        String reply1="";
        URI uri = null;
        try {
            uri = new URI("http://gabretailprod.cloudapp.net:8080/ShopDetails/retail/WebService/InsertShop");
